...
bool Collision::Box2DCollision(Vector2 playerPos, Vector2 enemyPos, float playerWidth, float enemyWidth)
{
	Vector2 playerMin = { playerPos.x - playerWidth,playerPos.y - playerWidth };
	Vector2 playerMax = { playerPos.x + playerWidth,playerPos.y + playerWidth };
	
	
	Vector2 EnemyMin = { enemyPos.x - enemyWidth,enemyPos.y - enemyWidth };
	Vector2 EnemyMax = { enemyPos.x + enemyWidth,enemyPos.y + enemyWidth };
	
	if ((playerMin.x <= EnemyMax.x && playerMax.x >= EnemyMin.x) && (playerMin.y <= EnemyMax.y && playerMax.y >= EnemyMin.y)) {
		return true;
	}
...
bool Collision::RaySphere(Vector3 rayPos, Vector3 enemyPos,float enemyRedius, Vector3 dir)
{
	Vector3 m = rayPos - enemyPos;
	float b = m.dot(dir);
	float c = m.dot(m) - (enemyRedius * enemyRedius);
	// layの始点がsphereの外側にあり(c > 0)、layがsphereから離れていく方向を
	// 差している場合(b > 0)、当たらない
	if (c > 0.0f && b > 0.0f)
	{
