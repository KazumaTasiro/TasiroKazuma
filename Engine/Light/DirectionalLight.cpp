...
void DirectionalLight::StaticInitialize(ID3D12Device* devices)
{
	//再初期化チェック
	assert(!DirectionalLight::device);
	//静的メンバ変数のセット
	DirectionalLight::device = devices;
}
...
{
	//正規化してセット
	this->lightdir = lightdirs.Normalization();
	dirty = true;
}

void DirectionalLight::SetLightColor(const Vector3& lightcolors)
{
	this->lightcolor = lightcolors;
	dirty = true;
}
