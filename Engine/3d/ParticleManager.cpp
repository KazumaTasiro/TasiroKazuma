...
void ParticleManager::Add(int life,Vector3 position,Vector3 velociy,Vector3 speed,float start_scale,float end_scale,int particleNmb)
{
	//リストに要素を追加
...
double ParticleManager::lerp(double a,double b,double t)
{
	return a + t * ( b - a );
}

void ParticleManager::SetColor(Vector4 color_)
{
	for ( std::forward_list<Particle>::iterator it = particles.begin();
	it != particles.end();
	it++ )
...
void ParticleManager::RandParticle()
{
	for ( int i = 0; i < 30; i++ )
	{
		...
		// 追加
		Add(60,pos,vel,acc,1.0f,0.0f,0);
	}
}