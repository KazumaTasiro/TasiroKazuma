...
ComPtr<ID3D12Device> ParticleManager2::device;
UINT ParticleManager2::descriptorHandleIncrementSize = 0;
ComPtr<ID3D12GraphicsCommandList> ParticleManager2::cmdList;
...
Matrix4 ParticleManager2::matBillboard = Affin::matUnit();
Matrix4 ParticleManager2::matBillboardY = Affin::matUnit();

Camera* ParticleManager2::camera = nullptr;

ParticleManager2::ParticleManager2() {
...
{
	// 3Dオブジェクトのインスタンスを生成
	ParticleManager2* ParticleManager = new ParticleManager2();
...
void ParticleManager2::InitializeGraphicsPipeline()
{
	...
	result = D3DCompileFromFile(
		L"Resources/shaders/Particle2VS.hlsl",	// シェーダファイル名
		nullptr,
	...
	result = D3DCompileFromFile(
		L"Resources/shaders/Particle2GS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
	...
	// ピクセルシェーダの読み込みとコンパイル
	result = D3DCompileFromFile(
		L"Resources/shaders/Particle2PS.hlsl",	// シェーダファイル名
		nullptr,
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルード可能にする
...
void ParticleManager2::LoadTexture(const std::string& fileName)
{
...
void ParticleManager2::Add(int life, Vector3 position, Vector3 velociy, Vector3 accel, float start_scale, float end_scale)
{
	//リストに要素を追加
